...
#include "bgfx/c99_imgui_impl/imgui.h"
...
#include "bgfx/vs_imgui_image.bin.h"
#include "bgfx/fs_imgui_image.bin.h"
...
static void *native_window_handle(void *window) {
  ...
  wl_egl_window *win_impl =
      (wl_egl_window *)SDL_GetWindowData(sdl_window, "wl_egl_window");
  if (!win_impl) {
    int width, height;
...
static void ImguiBgfxOnCreateWindow(ImGuiViewport *viewport) {
  ...
  data->frameBufferHandle = bgfx::createFrameBuffer(
      native_window_handle((SDL_Window *)viewport->PlatformHandle), data->width,
      data->height);
...
void ImGui_Implbgfx_RenderDrawLists(bgfx::ViewId viewId,
