...
static ecs_os_thread_t posix_thread_new(ecs_os_thread_callback_t callback,
                                        void *arg) {
...
static void *posix_thread_join(ecs_os_thread_t thread) {
  void *arg;
...
static int32_t posix_ainc(int32_t *count) {
  int value;
#ifdef __GNUC__
...
static int32_t posix_adec(int32_t *count) {
  int value;
#ifdef __GNUC__
...
static void posix_mutex_free(ecs_os_mutex_t m) {
  pthread_mutex_t *mutex = (pthread_mutex_t *)(intptr_t)m;
  pthread_mutex_destroy(mutex);
  ecs_os_free(mutex);
}

static void posix_mutex_lock(ecs_os_mutex_t m) {
  pthread_mutex_t *mutex = (pthread_mutex_t *)(intptr_t)m;
...
static void posix_mutex_unlock(ecs_os_mutex_t m) {
  pthread_mutex_t *mutex = (pthread_mutex_t *)(intptr_t)m;
