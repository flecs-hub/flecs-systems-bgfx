...
#define ECS_STRUCT_IMPL(name, descriptor, ...)                                 \
  ...
  ECS_UNUSED                                                                   \
  static EcsMetaType __##name##__ = {EcsStructType, sizeof(name),              \
                                     ECS_ALIGNOF(name), descriptor, NULL};

#define ECS_ENUM_IMPL(name, descriptor, ...)                                   \
  typedef enum name __VA_ARGS__ name;                                          \
  ECS_UNUSED                                                                   \
  static EcsMetaType __##name##__ = {EcsEnumType, sizeof(name),                \
                                     ECS_ALIGNOF(name), descriptor, NULL};

#define ECS_BITMASK_IMPL(name, descriptor, ...)                                \
  typedef enum name __VA_ARGS__ name;                                          \
  ECS_UNUSED                                                                   \
  static EcsMetaType __##name##__ = {EcsBitmaskType, sizeof(name),             \
                                     ECS_ALIGNOF(name), descriptor, NULL};
...
#define ECS_ARRAY(name, T, length)                                             \
  ...
  ECS_UNUSED                                                                   \
  static EcsMetaType __##name##__ = {EcsArrayType, sizeof(T) * length,         \
                                     ECS_ALIGNOF(T), "(" #T "," #length ")",   \
                                     NULL}

#define ECS_VECTOR(name, T)                                                    \
  typedef ecs_vector_t *name;                                                  \
  ECS_UNUSED                                                                   \
  static EcsMetaType __##name##__ = {EcsVectorType, sizeof(ecs_vector_t *),    \
                                     ECS_ALIGNOF(ecs_vector_t *), "(" #T ")",  \
                                     NULL}

#define ECS_MAP(name, K, T)                                                    \
  typedef ecs_map_t *name;                                                     \
  ECS_UNUSED                                                                   \
  static EcsMetaType __##name##__ = {EcsMapType, sizeof(ecs_map_t *),          \
                                     ECS_ALIGNOF(ecs_map_t *),                 \
                                     "(" #K "," #T ")", NULL}
...
#define ECS_ALIAS(type, name)                                                  \
  typedef type name;                                                           \
  ECS_UNUSED                                                                   \
  static EcsMetaType __##name##__ = {0, sizeof(name), ECS_ALIGNOF(name), NULL, \
                                     &__##type##__};
...
template <typename T> using bitmask = int32_t;
} // namespace flecs

#endif

ECS_ENUM_BOOTSTRAP(ecs_type_kind_t,
                   {EcsPrimitiveType, EcsBitmaskType, EcsEnumType,
                    EcsStructType, EcsArrayType, EcsVectorType, EcsMapType})
...
ECS_ENUM(ecs_primitive_kind_t,
         {EcsBool, EcsChar, EcsByte, EcsU8, EcsU16, EcsU32, EcsU64, EcsI8,
          EcsI16, EcsI32, EcsI64, EcsF32, EcsF64, EcsUPtr, EcsIPtr, EcsString,
          EcsEntity})

ECS_STRUCT(EcsPrimitive, { ecs_primitive_kind_t kind; })
...
ECS_STRUCT(EcsBitmask, { flecs::map<int32_t, flecs::string_t> constants; })
#else
ECS_STRUCT(EcsBitmask, { ecs_map(int32_t, ecs_string_t) constants; })
#endif

// Define EcsEnum for both C and C++. Both representations are equivalent in
// memory, but allow for a nicer type-safe API in C++
#if defined(__cplusplus) && !defined(FLECS_NO_CPP)
ECS_STRUCT(EcsEnum, { flecs::map<int32_t, flecs::string_t> constants; })
#else
ECS_STRUCT(EcsEnum, { ecs_map(int32_t, ecs_string_t) constants; })
#endif
...
ECS_STRUCT(EcsVector, { ecs_entity_t element_type; })
...
ECS_ENUM_C(ecs_type_op_kind_t,
           {EcsOpHeader, EcsOpPrimitive, EcsOpEnum, EcsOpBitmask, EcsOpPush,
            EcsOpPop, EcsOpArray, EcsOpVector, EcsOpMap})
...
ECS_STRUCT_C(EcsMetaTypeSerializer, { ecs_vector(ecs_type_op_t) ops; })
...
FLECS_META_API
char *ecs_ptr_to_str(ecs_world_t *world, ecs_entity_t type, void *ptr);
...
/** Escape a character */
FLECS_META_API
char *ecs_chresc(char *out, char in, char delimiter);
...
/** Escape a string */
FLECS_META_API
ecs_size_t ecs_stresc(char *out, ecs_size_t n, char delimiter, const char *in);
...
FLECS_META_API
ecs_meta_cursor_t ecs_meta_cursor(const ecs_world_t *world, ecs_entity_t type,
                                  void *base);
...
FLECS_META_API
int ecs_meta_move(ecs_meta_cursor_t *cursor, int32_t pos);

FLECS_META_API
int ecs_meta_move_name(ecs_meta_cursor_t *cursor, const char *name);
...
FLECS_META_API
void ecs_new_meta(ecs_world_t *world, ecs_entity_t component,
...
template <typename T> flecs::entity meta(flecs::world &world) {
...
template <typename T> flecs::string pretty_print(flecs::world &world, T &data) {
  entity_t type = _::cpp_type<T>::id();
  return flecs::pretty_print(world, type, data);
}

template <>
inline flecs::string pretty_print<flecs::entity>(flecs::world &world,
                                                 flecs::entity &entity) {
